...
const useMatter = (canvasRef, width, height, onItemDropped, getSortingZoneRect, items) => {
  ...
  useEffect(() => {
    ...
    function getBodyAtPoint(event) {
      ...
      const x = (event.clientX || (event.touches && event.touches[0] ? event.touches[0].clientX : 0)) - canvasRect.left;
      const y = (event.clientY || (event.touches && event.touches[0] ? event.touches[0].clientY : 0)) - canvasRect.top;
      ...
      for (let i = 0; i < engine.world.bodies.length; i++) {
        ...
        if (body.label.startsWith('item-') || body.isDraggable) {
          const dx = body.position.x - x;
          const dy = body.position.y - y;
          const distance = dx * dx + dy * dy; // 使用距离平方，避免开平方操作
      ...
      // 只对最近的几个物体进行精确的边界检测
      for (let i = 0; i < closestBodies.length; i++) {
        const body = closestBodies[i].body;
    ...
    // 添加点击事件监听
    // render.canvas.addEventListener('click', handleClick); // 移除重复的click监听器
  ...
  }, [canvasRef, width, height, onItemDropped, getSortingZoneRect, bodies]);
